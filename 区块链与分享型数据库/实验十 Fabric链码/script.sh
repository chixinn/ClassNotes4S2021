...
instantiateChaincode () {
	...
	# {"Args":["init","idsum","idmax","idmin","sum","max","min"]}
		peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc -v 1.0 -c '{"Args":["init","idsum","idmax","idmin","0","0","0"]}' -P "OR	('Org1MSP.peer','Org2MSP.peer')" >&log.txt
	else
		peer chaincode instantiate -o orderer.example.com:7050 --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -v 1.0 -c '{"Args":["init","idsum","idmax","idmin","0","0","0"]}' -P "OR	('Org1MSP.peer','Org2MSP.peer')" >&log.txt
	fi
	res=$?
	cat log.txt
...
chaincodeInsert () {
  PEER=$1
  echo "===================== Inserting on PEER$PEER on channel '$CHANNEL_NAME'... ===================== "
  setGlobals $PEER
  local rc=1
  local starttime=$(date +%s)
  ...
  do
     sleep 3
     echo "Attempting to Query PEER$PEER ...$(($(date +%s)-starttime)) secs"
     peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["insert","a","100"]}' >&log.txt
     test $? -eq 0 && VALUE=$(cat log.txt | awk '/Insert Result/ {print $NF}')
     test "$VALUE" = "$2" && let rc=0
  done
  echo
  cat log.txt
...
#Insert on chaincode on Peer0/Org1
echo "Inserting chaincode on org1/peer0..."
chaincodeInsert 0

#Insert on chaincode on Peer2/Org2
echo "Inserting chaincode on org2/peer2..."
chaincodeQueryInsert 2
